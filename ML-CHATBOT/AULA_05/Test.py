...
# print(f"SOLUÇÃO: Previsão de preço para um imóvel de 100m² com 3 quartos: R$ {preco_previsto[0]:.2f} mil")
# print("-" * 50, "\n")
...
# print(f"SOLUÇÃO: Clusters para as transações: {clusters_transacoes}")
# anomalia_index = np.where(np.bincount(clusters_transacoes) == 1)[0][0]
# print(f"A anomalia está no cluster {anomalia_index}, que corresponde à transação: \n{transacoes[clusters_transacoes == anomalia_index]}")
# print("-" * 50, "\n")
...
# ==============================================================================
# EXERCÍCIO 5 - APRENDIZADO POR REFORÇO: O AGENTE COMILÃO
...
print("--- Iniciando a Simulação do Agente Guloso ---")
...
# Exibe o resultado do passo (JÁ ESTÁ PRONTO)
    print(f"   <- Resposta do Ambiente: Nova Posição={posicao_agente}, Recompensa={recompensa_do_passo}")
        
# 4. VERIFIQUE SE O JOGO TERMINOU
#    Se a condição de vitória foi atingida, paramos a simulação.
    if posicao_agente == POSICAO_COMIDA:
        print("\nO agente encontrou a comida!")
        break
